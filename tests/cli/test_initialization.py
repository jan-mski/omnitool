...
def test_initialize_operations_with_single_plugin(mocker,
                                                  mock_loaded_plugins, 
    ...
    actual_command_names = sorted([command.name for command in app.registered_groups[0].typer_instance.registered_commands])
...
def test_initialize_operations_with_multiple_plugins(mocker,
                                                     mock_loaded_plugins,
                                                     create_loaded_plugin,
                                                     mock_cli_operation):
    """
    ...
    actual_plugin1_command_names = sorted([command.name for command in app.registered_groups[0].typer_instance.registered_commands])

    expected_plugin2_command_names = sorted(plugin2_operation_names)
    actual_plugin2_command_names = sorted([command.name for command in app.registered_groups[1].typer_instance.registered_commands])
    
    expected_calls = (
        [mocker.call(plugin1, operation) for operation in plugin1.definition.operations] +
